...
def test_estimates_align_with_runtime(request, profile_settings, profile_report_enabled):
    ...
    act_ws_bytes = max(peak_bytes - param_bytes - kv_cache_bytes, 0)

    checks: list[tuple[str, float, float, float]] = [
        ("parameters",
         _bytes_to_gib(param_bytes),
         estimate.parameters.lower_gib,
         estimate.parameters.upper_gib),
        ("kv_cache",
         _bytes_to_gib(kv_cache_bytes),
         estimate.kv_cache.lower_gib,
         estimate.kv_cache.upper_gib),
        ("act+workspace",
         _bytes_to_gib(act_ws_bytes),
         estimate.activations.lower_gib + estimate.workspace.lower_gib,
         estimate.activations.upper_gib + estimate.workspace.upper_gib),
        ("total",
         _bytes_to_gib(total_bytes),
         estimate.total.lower_gib,
         estimate.total.upper_gib),
    ]

    report_lines = []
    for label, actual_gib, lower, upper in checks:
        msg = (
            f"{label} actual {actual_gib:.3f} GiB outside range "
            f"({lower:.3f}, {upper:.3f})"
        )
        report_lines.append(
            f"{label:<14} actual {actual_gib:6.3f} GiB | "
            f"[{lower:6.3f}, {upper:6.3f}]"
        )
        assert lower <= actual_gib <= upper, msg
